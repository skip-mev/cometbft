...
func TestHandshake(t *testing.T) {
	...
	go func() {
		...
		_, err := handshake(nodeInfo, c1, 20*time.Millisecond, nil)
		if err != nil {
	...
	_, err := handshake(nodeInfo, c2, 20*time.Millisecond, nil)
...
func TestTransportMultiplexRejectSelf(t *testing.T) {
	...
	go func() {
		...
		_, err := handshake(nodeInfo2, c1, 20*time.Millisecond, nil)
		if err == nil {
			panic("expected handshake to fail")
		}
	}()

	_, err := handshake(nodeInfo1, c2, 20*time.Millisecond, nil)
	require.Error(t, err)
